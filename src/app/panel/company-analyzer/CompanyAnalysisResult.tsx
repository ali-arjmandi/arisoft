...
import { buildCompanyReportHtml } from "@/lib/companyAnalyzer/companyReportHtml";
...
export function CompanyAnalysisResult({ result }: { result: CompanyAnalysis }) {
  ...
  function handleGenerateEmail() {
    ...
    const newTab = window.open("/panel/email?awaiting=1", "_blank");
    if (!newTab) {
    ...
    void (async () => {
      ...
      try {
        const res = await fetch("/api/panel/generate-company-email", {
          method: "POST",
