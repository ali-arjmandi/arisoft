...
export function parseSendRequest(body: unknown): ParseResult {
  ...
  const companyId = readOptionalId(record.companyId);
  const contactPersonId = readOptionalId(record.contactPersonId);

  return { ok: true, data: { to, from: from as AllowedSender, content, companyId, contactPersonId } };
}
